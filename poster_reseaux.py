...
def publier_instagram(image_url, legende):
    ...
    for attempt in range(1, 6):
        ...
        subcode = None
        try:
            subcode = r.json().get("error", {}).get("error_subcode")
        except Exception:
            pass
        media_pas_pret = (r.status_code == 400 and subcode == 2207027)

        if r.status_code < 500 and not media_pas_pret:
            break
        if attempt < 5:
            attente = 30 if r.status_code >= 500 else 10
            print(f"│  ⏳ Tentative {attempt}/5 échouée ({last_err}) — nouvel essai dans {attente} s…")
            time.sleep(attente)
